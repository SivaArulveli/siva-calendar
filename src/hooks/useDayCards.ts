...
function migrateOldCard(card: OldCard | DayCard): DayCard {
  if ("tamil_date" in card) return card as DayCard; // already new format
  ...
  return {
    ...
    events: (card.events || []).map((e: { name: string }) => ({
      title_ta: e.name,
      title_en: e.name,
      description_ta: "",
      description_en: "",
    ...
    const loadCards = async () => {
      try {
        const response = await fetch(
          resolveAsset("/data/day_cards.json") || "/data/day_cards.json",
        );
        let data: { dayCards?: DayCard[] | OldCard[]; days?: DayCard[] | OldCard[] } = {
          dayCards: [],
        };
        if (response.ok) {
