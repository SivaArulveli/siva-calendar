...
export function DayCardForm({
  ...
  const addEvent = () => {
    setFormData((prev) => ({
      ...prev,
      events: [...(prev.events || []), { title_ta: "", title_en: "", description_ta: "", description_en: "", importance_level: "medium", icon: "calendar" }],
    }));
  };
  ...
  const inputClass = "bg-white border-[#dadce0] text-[#202124] placeholder:text-[#9aa0a6] focus:border-[#1a73e8] focus:ring-[#1a73e8]/20";
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="bg-[#e8f0fe] border border-[#c5d8fc] rounded-xl p-4 space-y-4">
        <p className="text-xs font-bold uppercase tracking-widest text-[#1a73e8]">Tamil Date (தமிழ் தேதி)</p>
        ...
        <div className="bg-[#f8f9fa] border border-[#dadce0] rounded-xl p-4 space-y-4">
           ...
           <div className="space-y-1.5">
            ...
            <Input
              ...
              onChange={(e) => handleNestedChange("visual", "placeholder_image_url", e.target.value)}
              placeholder="https://..."
              className={inputClass}
            />
          </div>
      ...
      <div className="space-y-3">
        ...
        {formData.events?.map((event, index) => (
          <div key={index} className="flex flex-col gap-3 bg-[#f8f9fa] p-4 rounded-xl border border-[#dadce0]">
            <div className="flex justify-between items-center mb-1">
              <span className="text-xs font-bold text-[#5f6368]">Event #{index + 1}</span>
              <Button type="button" variant="ghost" size="icon" className="h-6 w-6 text-destructive hover:text-destructive/80 hover:bg-destructive/10" onClick={() => removeEvent(index)}>
                <Trash2 className="w-3.5 h-3.5" />
              </Button>
            </div>
            
            <div className="grid grid-cols-2 gap-3">
              <Input placeholder="Title (Tamil)" value={event.title_ta} onChange={(e) => handleEventChange(index, "title_ta", e.target.value)} className={`h-9 text-sm tamil-font ${inputClass}`} required />
              <Input placeholder="Title (English)" value={event.title_en} onChange={(e) => handleEventChange(index, "title_en", e.target.value)} className={`h-9 text-sm ${inputClass}`} required />
            </div>
            
            <div className="grid grid-cols-2 gap-3">
              <Textarea placeholder="Description (Tamil)" value={event.description_ta || ""} onChange={(e) => handleEventChange(index, "description_ta", e.target.value)} className={`text-sm tamil-font ${inputClass}`} rows={2} />
              <Textarea placeholder="Description (English)" value={event.description_en || ""} onChange={(e) => handleEventChange(index, "description_en", e.target.value)} className={`text-sm ${inputClass}`} rows={2} />
            </div>
            
            <div className="grid grid-cols-2 gap-3">
              <Input placeholder="Importance (high, medium, low)" value={event.importance_level || ""} onChange={(e) => handleEventChange(index, "importance_level", e.target.value)} className={`h-9 text-sm ${inputClass}`} />
              <Input placeholder="Icon Identifier" value={event.icon || ""} onChange={(e) => handleEventChange(index, "icon", e.target.value)} className={`h-9 text-sm ${inputClass}`} />
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="flex gap-3 justify-end pt-4 border-t border-[#dadce0]">
        {onDelete && (
          <Button type="button" variant="destructive" onClick={() => onDelete(initialData.id)} className="mr-auto">
            Delete Card
          </Button>
        )}
        <Button type="button" variant="ghost" onClick={onCancel} className="text-[#5f6368] hover:text-[#202124] hover:bg-[#f1f3f4]">
          Cancel
        </Button>
