...
function YearMonthSwitcher({ yearIdx, monthIdx, onYearChange, onMonthChange }: SwitcherProps) {
  ...
  const prev = () => {
    if (monthIdx === 0) {
      onMonthChange(11);
      onYearChange((yearIdx - 1 + 60) % 60);
    } else onMonthChange(monthIdx - 1);
  };
  const next = () => {
    if (monthIdx === 11) {
      onMonthChange(0);
      onYearChange((yearIdx + 1) % 60);
    } else onMonthChange(monthIdx + 1);
  };

  const selectClass =
    "appearance-none bg-transparent border-none text-[#202124] text-sm focus:outline-none cursor-pointer pr-4 font-bold";
        ...
        <div className="flex flex-col relative group">
          <div className="flex items-center">
            <select
              ...
              className={`${selectClass} text-xl sm:text-2xl text-[#1a73e8] tamil-font tracking-wide`}
            >
              {TAMIL_MONTHS.map((m, i) => (
                <option key={i} value={i} className="text-[#202124]">
                  {m.ta}
                </option>
              ))}
            </select>
          </div>
          <span className="text-[#5f6368] text-xs font-semibold uppercase tracking-widest">
            {month.en}
          </span>
        </div>
        ...
        <div className="flex flex-col relative group mt-1 sm:mt-0">
          <div className="flex items-center">
            <select
              ...
              className={`${selectClass} text-lg sm:text-xl text-[#1a73e8] tamil-font`}
            >
          ...
          <span className="text-[#5f6368] text-[10px] uppercase tracking-wider">
            {year.en} Year
          </span>
        </div>
      </div>
...
function DayCell({
  ...
  if (cell.isPadding) {
    return (
      <div className="rounded-xl border border-transparent bg-transparent opacity-30 h-full min-h-[140px]" />
    );
  }
          ...
          <div className="flex justify-between items-start p-2 border-b border-[#dadce0]/50 bg-[#f8f9fa]">
            <div className="flex flex-col">
              <span className="text-xs font-bold text-[#1a73e8] tamil-font flex items-center gap-1">
                {cell.tamilDay}{" "}
                <span className="bg-[#e8f0fe] text-[#1a73e8] text-[8px] px-1 rounded font-medium border border-[#c5d8fc]">
                  தமிழ்
                </span>
              </span>
            </div>
            <div className="flex flex-col items-end">
              <span className="text-xs font-mono text-[#5f6368] flex items-center gap-1">
                <span className="bg-[#f1f3f4] text-[#5f6368] text-[8px] px-1 rounded border border-[#dadce0]">
                  EN
                </span>{" "}
                {cell.gregDay}
              </span>
            ...
            {card.events && card.events.length > 0 && (
              <div className="mb-2">
                <div className="flex items-center gap-1">
                  {card.events[0].importance_level === "high" && (
...
export function DayCardsDashboard() {
  ...
  end.setUTCDate(end.getUTCDate() + meta.days - 1);
  const startStr = start.toISOString().split("T")[0];
  const endStr = end.toISOString().split("T")[0];

  const cardMap = useMemo(() => {
    const m: Record<string, DayCard> = {};
    // Only include cards that fall within the Gregorian range of this Tamil month
    for (const c of dayCards) {
      if (c.id >= startStr && c.id <= endStr) {
        m[c.id] = c;
      }
    }
    return m;
  }, [dayCards, startStr, endStr]);
  ...
  const cells: CellData[] = useMemo(() => {
    ...
    for (let d = 0; d < meta.days; d++) {
      ...
      out.push({
        ...
        dayCard: card, // card is already filtered in cardMap
        isPadding: false,
        tamilDay: tamilDay,
      });
    }
    ...
    return out;
  }, [meta, start, cardMap, today]);

  const ymeta = TAMIL_YEARS_60[yearIdx];
  // const end was already defined above cardMap
  ...
  const handleCellClick = (cell: CellData) => {
    ...
    if (cell.dayCard) {
      ...
      setSelectedCard({
        ...
        gregorian_date: {
          iso: cell.gregDate,
          display_en: dt.toLocaleDateString("en-US", {
            month: "long",
            day: "numeric",
            year: "numeric",
          }),
        ...
        visual: {
          placeholder_image_url: randomImage,
    ...
    <div className="w-full max-w-6xl mx-auto space-y-6 px-2 sm:px-4 pb-12 pt-6">
            ...
            {isAdmin ? (
              <>
                <Button
                  ...
                  className="bg-[#1a73e8] text-white hover:bg-[#1557b0] text-xs gap-1.5 shadow-sm border-none rounded-lg"
                >
                ...
                <Button
                  ...
                  variant="outline"
                  className="border-[#dadce0] text-[#5f6368] hover:bg-[#f8f9fa] hover:text-[#202124] text-xs gap-1.5 bg-white rounded-lg"
                >
      ...
      <div className="grid grid-cols-7 gap-2">
        {TAMIL_WEEKDAYS.map((d, i) => (
          <div
            key={i}
            className="flex flex-col items-center py-2 px-1 rounded-lg bg-white border border-[#dadce0] shadow-sm"
          >
            <span className="tamil-font text-[#1a73e8] text-sm font-bold">{d.ta}</span>
            <span className="text-[#5f6368] text-[10px] uppercase font-semibold tracking-wider mt-0.5">
              {d.en}
            </span>
          </div>
        ))}
